...
class GeminiService:
    ...
    def _generate(self, prompt: str, image_paths: list[str] | None = None, max_tokens: int = 4096) -> str:
        if not self.client:
...
Respond in JSON with keys: page_name, purpose, primary_action, leads_to, journey_moment, is_key_moment."""
        ...
        return extract_json(self._generate(prompt, image_paths=images, max_tokens=1024))
...
Respond in JSON with keys: changes, change_type, affected_flows, severity."""
        ...
        return extract_json(self._generate(prompt, image_paths=[old_path, new_path], max_tokens=1024))
...
Respond in JSON with keys: merged_nodes, merged_edges, change_summary."""
    ...
    def check_health(self) -> dict:
        ...
        try:
            self._generate("Reply with exactly: OK", max_tokens=16)
            return {"status": "ok"}
